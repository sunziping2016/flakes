...
let
  env = {
    ...
    # MAIL_PORT = "<env_file>";
    # MAIL_FROM = "<env_file";
    # MAIL_USERNAME = "<env_file>";
    # MAIL_PASSWORD = "<env_file>";
    ...
    #
    APP_URL = "https://firefly.szp15.com";
  };
in
{
  sops.secrets = {
  ...
  virtualisation.arion.projects.firefly.settings = {
    services = {
      app = {
        service.image = "fireflyiii/core:latest";
        service.volumes = [
          "firefly_iii_upload:/var/www/html/storage/upload"
        ];
        service.ports = [
          "127.0.0.1:18098:8080"
        ];
        service.env_file = [
          config.sops.templates."firefly_app_env_file".path
        ];
      ...
      cron = {
        ...
        service.command = "sh -c 'echo \"0 3 * * * wget -qO- http://app:8080/api/v1/cron/$$STATIC_CRON_TOKEN\" | crontab - && crond -f -L /dev/stdout'";
        service.environment = {
          TZ = "Asia/Shanghai";
        };
        service.env_file = [
          config.sops.templates."firefly_cron_env_file".path
        ];
        service.depends_on = [ "app" ];
        service.restart = "unless-stopped";
      };
      backup = {
        service.image = "fradelg/mysql-cron-backup:latest";
        service.depends_on = [ "db" ];
        service.volumes = [
          "/srv/firefly/backup:/backup"
        ];
        service.environment = {
          MYSQL_HOST = "db";
          MYSQL_USER = "firefly";
  ...
  systemd.services.arion-firefly = {
